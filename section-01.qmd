...
1.  Use the `filter()` function to remove all NAs from the `generation` data object and keep only cities with population greater than 100,000.

2.  Store the result as a new object in your environment with the name `generation_filtered`.
...
1.  Starting with `waste_per_capita`, use a function of the slice family of functions to return the 8 cities with the lowest municipal solid waste generation per capita.
